...
<img src="screenshots/minimal-ui.png" alt="Waveform navigator showing only the waveform visualization without playback controls" width="800"/>

Waveform only (`showControls={false}`), perfect for custom UI implementations using the component's ref API.
...
- **`defaultVolume`** (number, default: 1): Initial volume on mount, clamped to 0–1. Use this to restore a previously persisted level (e.g. from `localStorage`). The value is only read once on mount; it does not update the volume if changed after render.
- **`onVolumeChange`** ((volume: number) => void): Callback fired whenever the user changes the volume via the slider or mute/unmute toggle. Receives the new clamped value (0–1). Use this alongside `defaultVolume` to persist and restore volume across sessions.
