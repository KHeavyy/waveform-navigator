...
describe('useAudioPlayer', () => {
  it('responds to play/pause events and exposes controls', async () => {
    ...
    await waitFor(() => expect(screen.getByTestId('isPlaying').textContent).toBe('true'))

    // Simulate pause event
    audioEl!.dispatchEvent(new Event('pause'))
    await waitFor(() => expect(screen.getByTestId('isPlaying').textContent).toBe('false'))

    // Simulate timeupdate
    // Set currentTime and dispatch event
    audioEl!.currentTime = 3
    audioEl!.dispatchEvent(new Event('timeupdate'))
    await waitFor(() => expect(screen.getByTestId('currentTime').textContent).toContain('3'))
  })
  ...
  it('supports controlled mode and seekTo notifies parent', async () => {
    ...
    await waitFor(() => expect(screen.getByTestId('isControlled').textContent).toBe('true'))
