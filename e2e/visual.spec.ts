...
test.describe('Visual Regression Tests', () => {
	...
	test('should match waveform snapshot at DPR 1', async ({ page }, testInfo) => {
		...
		const snapshotPath = path.join(
			SNAPSHOT_DIR,
			`${testInfo.project.name}-waveform.png`
		);
		...
		if (!fs.existsSync(snapshotPath) || process.env.UPDATE_SNAPSHOTS) {
			...
			if (diffPercentage > THRESHOLD) {
				...
				const diffPath = path.join(
					SNAPSHOT_DIR,
					`${testInfo.project.name}-waveform-diff.png`
				);
				...
				const currentPath = path.join(
					SNAPSHOT_DIR,
					`${testInfo.project.name}-waveform-current.png`
				);
	...
	test('should render consistently at different widths', async ({ page }) => {
		...
		if (box1 && box2 && Math.abs(box1.width - box2.width) > 1) {
			...
			try {
				...
				if (numDiffPixels === 0) {
					console.log(
						'Bounding box changed but images are identical (0 diff pixels); skipping image-difference assertion'
					);
				} else {
			...
			console.log(
				'Canvas bounding box did not change after viewport resize; skipping image-difference assertion'
			);
		}
	});
